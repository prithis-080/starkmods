...
ADMIN_IDS = [7031907806, 5175810761]
BOT_TOKEN = "7084291622:AAFnJSFabolZKIreZcl7iqCNk3x9QS8d8iI"
thread_count = 500
packet_size = 12
...
ALLOWED_GROUP_ID = -1002439444020  # Replace with your actual group ID
...
def send_owner_info(message):
    ...
    owner_message = "This Bot Has Been Developed By ᚛ @MR_ADARSH_YTs ᚜"  
...
def send_welcome(message):
    ...
    if is_super_admin(user_id):
        welcome_message = (
            f"Welcome, Super Admin! Developed By ᚛ @MR_ADARSH_YTs ᚜\n\n"
            f"Admin Commands:\n"
        ...
        balance = get_admin_balance(user_id)
        welcome_message = (
            f"Welcome, Admin! Developed By ᚛ @MR_ADARSH_YTs ᚜\n\n"
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*𝐘𝐨𝐮 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @MR_ADARSH_YTs ᚜*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @MR_ADARSH_YTs ᚜.*", parse_mode='Markdown')
        return
    ...
    if is_super_admin(user_id):
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = (
                ...
                "Contact ᚛ @MR_ADARSH_YTs ᚜ for assistance."
            )
        else:
            account_info = (
        ...
        account_info = "𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @MR_ADARSH_YTs ᚜."
